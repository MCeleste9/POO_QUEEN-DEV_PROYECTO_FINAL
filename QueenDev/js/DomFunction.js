...
dropArea.addEventListener("dragover", (event)=>{
  ...
  //Si la usuario deja el archivo arrastrado desde DropArea
  ...
  function generate(l) {
    ...
    /* c : cadena de caracteres alfanuméricos */
    var c='abcdefghijknopqrstuvwxyzACDEFGHJKLMNPQRSTUVWXYZ',
    n=c.length,
    /* p : cadena de caracteres especiales */
    p='-_',
    o=p.length,
    r='',
    n=c.length,
    /* s : determinar la posición del carácter especial */
    s=Math.floor(Math.random() * (p.length-1));

    for(var i=0; i<l; ++i){
        if(s == i){
            /* inserción de caracteres especiales (posición aleatoria) */
            r += p.charAt(Math.floor(Math.random() * o));
        }else{
            /* inserción alfanumérica */
